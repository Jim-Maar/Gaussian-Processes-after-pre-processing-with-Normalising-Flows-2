...
class Conv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.weight.data.normal_(mean=0.0, std=weight_std).type(torch.DoubleTensor)
        if not do_actnorm:
    ...
    def forward(self, input):
        input=input.type(torch.DoubleTensor).cuda() 
...
class Conv2dZeros(nn.Conv2d):
    def __init__(self, in_channels, out_channels,
        ...
        self.register_parameter("logs", nn.Parameter(torch.zeros(out_channels, 1, 1).type(torch.DoubleTensor)))
        # init
