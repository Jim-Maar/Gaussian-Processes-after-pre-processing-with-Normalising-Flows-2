    ...
    def forward(self, input: torch.Tensor, logdet=0, reverse=False, permute=False):
     if not reverse:
        ...
        attn = (s((torch.matmul(full_inp_q1, full_inp_k1.permute(0, 2,
            ...
            1))/self.scale)+(torch.matmul(full_inp_q6, full_inp_k6.permute(0, 2,1))/self.scale)+(torch.matmul(full_inp_q7, full_inp_k7.permute(0, 2,1))/self.scale)+self.offset2)+self.offset3)*attn_mask #Compute attention (masked) attn_mask =
