...
class Transformer_attn(nn.Module):
    def __init__(self, num_channels):
        ...
        self.register_parameter("offset2",nn.Parameter(torch.ones([1,1,1])*0.58))
        #self.off3= torch.nn.Parameter(torch.ones([1],dtype=torch.double)*(6),requires_grad=True).cuda()
        self.register_parameter("offset3",nn.Parameter(torch.ones([1,1,1])*2))
        self.register_parameter("scale",nn.Parameter(torch.ones([1,1,1])*2000))
    ...
    def forward(self, input: torch.Tensor, logdet=0, reverse=False, permute=False):
     if not reverse:
        ...
        attn = (s((torch.matmul(full_inp_q1, full_inp_k1.permute(0, 2,
            ...
            1))/self.scale)+(torch.matmul(full_inp_q4, full_inp_k4.permute(0, 2,
            1))/self.scale)+(torch.matmul(full_inp_q5, full_inp_k5.permute(0, 2,
            1))/self.scale)+(torch.matmul(full_inp_q6, full_inp_k6.permute(0, 2,
            1))/self.scale)+(torch.matmul(full_inp_q7, full_inp_k7.permute(0, 2,
            1))/self.scale)+(torch.matmul(full_inp_q8, full_inp_k8.permute(0, 2,
            1))/self.scale)))*attn_mask #Compute attention (masked) attn_mask =
        ...
        attn = (s((torch.matmul(full_inp_q1_rev, full_inp_k1_rev.permute(0, 2, 1))/self.scale)+(torch.matmul(full_inp_q2_rev, full_inp_k2_rev.permute(0, 2, 1))/self.scale)+(torch.matmul(full_inp_q3_rev, full_inp_k3_rev.permute(0, 2, 1))/self.scale)+(torch.matmul(full_inp_q4_rev, full_inp_k4_rev.permute(0, 2, 1))/self.scale)+(torch.matmul(full_inp_q5_rev, full_inp_k5_rev.permute(0, 2, 1))/self.scale)+(torch.matmul(full_inp_q6_rev, full_inp_k6_rev.permute(0, 2, 1))/self.scale)+(torch.matmul(full_inp_q7_rev, full_inp_k7_rev.permute(0, 2, 1))/self.scale)+(torch.matmul(full_inp_q8_rev, full_inp_k8_rev.permute(0, 2, 1))/self.scale)))*attn_mask
