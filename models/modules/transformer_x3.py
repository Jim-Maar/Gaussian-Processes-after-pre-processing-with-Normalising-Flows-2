...
class Transformer_attn(nn.Module):
    ...
    def forward(self, input: torch.Tensor, logdet=0, reverse=False, permute=False):
     if not reverse:
        ...
        attn = (s((torch.matmul(full_inp_q1_rev, full_inp_k1_rev.permute(0, 2, 1))/self.scale)+(torch.matmul(full_inp_q2_rev, full_inp_k2_rev.permute(0, 2, 1))/self.scale)+(torch.matmul(full_inp_q3_rev, full_inp_k3_rev.permute(0, 2, 1))/self.scale)))*attn_mask
