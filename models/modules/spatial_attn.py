...
class _Spatial_first_order_attn(nn.Module):
    def __init__(self, input_channels):
        ...
        self.weight = torch.empty([self.input_channels,self.input_channels,1]).type(torch.FloatTensor).cuda()
        nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        self.weight=torch.nn.Parameter(self.weight).cuda()
        self.bias = torch.empty([self.input_channels]).type(torch.FloatTensor).cuda()
