...
from lstm import ConvSeqEncoder
...
class ChannelPriorUniScale(nn.Module):
    ...
    def get_likelihood(self, z):
        ...
        if self.level != self.tot_levels:
            init_zero_input = torch.zeros(
                self.batch_size, 1, 1, self.height, self.width)
        else:
            init_zero_input = torch.zeros(
                self.batch_size, 1, 1, self.height, self.width)
        ...
        seq_lengths = torch.LongTensor(
            (np.ones((self.batch_size,))*(in_ch_length)).astype(np.int32))  # .cuda()
    ...
    def get_sample(self, z1=None):
        with torch.no_grad():
            ...
            seq_lengths = torch.LongTensor(
                (np.ones((self.batch_size,))).astype(np.int32))
            ...
            for _ in range(self.nc):
                z_mean_logs, hidden = self.prior_lstm(
                    lstm_input, seq_lengths, hidden)
