...
from models.modules.spatial_attn import _Spatial_first_order_attn
class FlowStep(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels,
        ...
        #self.attn1=Transformer_attn(num_channels)
        #self.attn2=Transformer_attn(num_channels)
    ...
    def normal_flow(self, z, logdet, rrdbResults=None):
        ...
        #z,logdet=self.attn1(z,logdet=logdet,reverse=False)
        #z,logdet=self.attn2(z,logdet=logdet,reverse=False,permute=True)
    ...
    def reverse_flow(self, z, logdet, rrdbResults=None):
        ...
        need_features = self.affine_need_features()
        #z,logdet=self.attn2(z,logdet=logdet,reverse=True,permute=True)
        #z,logdet=self.attn1(z,logdet=logdet,reverse=True)
        # 1.coupling
        z,logdet=self.attn2(z,logdet=logdet,reverse=True,permute=True)
        z,logdet=self.attn1(z,logdet=logdet,reverse=True)
        if need_features or self.flow_coupling in ["condAffine", "condFtAffine", "condNormAffine"]:
