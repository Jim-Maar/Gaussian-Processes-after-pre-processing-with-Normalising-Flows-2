...
class _Spatial_first_order_attn(nn.Module):
    def __init__(self, input_channels,num_channels):
        ...
        self.num_channels=num_channels
        self.input_channels=input_channels
        self.weight = torch.empty([self.input_channels,self.input_channels,1]).type(torch.DoubleTensor).cuda()
        nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        self.weight=torch.nn.Parameter(self.weight,requires_grad=True)#.cuda()
        self.bias = torch.empty([self.input_channels]).type(torch.DoubleTensor).cuda()
        fan_in, _ = nn.init._calculate_fan_in_and_fan_out(self.weight)
        bound = 1 / math.sqrt(fan_in)
        nn.init.uniform_(self.bias, -bound, bound)
        self.bias =torch.nn.Parameter(self.bias,requires_grad=True)#.cuda()
        self.s = torch.nn.Parameter(torch.randn([1,self.input_channels,num_channels]),requires_grad=True)
        self.pool1 = torch.nn.AvgPool1d(self.input_channels)
