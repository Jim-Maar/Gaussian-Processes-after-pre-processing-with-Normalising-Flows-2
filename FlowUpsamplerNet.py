        ...
        for k in range(K):
            ...
            self.layers.append(
                FlowStep(in_channels=self.C,
                         ...
                         LU_decomposed=LU_decomposed, opt=opt, idx=k, normOpt=normOpt,inpshape=[self.C,H,W]))
            ...
            for _ in range(n_additionalFlowNoAffine):
                self.layers.append(
                    FlowStep(in_channels=self.C,
                             ...
                             LU_decomposed=LU_decomposed, opt=opt,inpshape=[self.C,H,W]))
                self.output_shapes.append(
                    [-1, self.C, H, W])
    ...
    def encode(self, gt, rrdbResults, logdet=0.0, epses=None, y_onehot=None):
        ...
        for layer, shape in zip(self.layers, self.output_shapes):
            ...
            if isinstance(layer, FlowStep):
                print("Nan",torch.sum(torch.isnan(fl_fea)))
