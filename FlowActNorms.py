...
class _ActNorm(nn.Module):
    ...
    def __init__(self, num_features, scale=1.):
        ...
        self.register_parameter("bias", nn.Parameter(torch.zeros(*size).double()).type(torch.DoubleTensor))
        self.register_parameter("logs", nn.Parameter(torch.zeros(*size).double()).type(torch.DoubleTensor))
