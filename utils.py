...
def get_dataset( dataset_name, batch_size, data_root=None, train_workers=4, test_workers=2 ):
	...
	if dataset_name == 'cifar10':
		...
		trainset = dsets.MNIST(root=data_root, train=True, download=False, transform=transform_train)
		train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True,drop_last=True,num_workers=train_workers)

		testset = dsets.MNIST(root=data_root, train=False, download=False, transform=transform_test)
		...
		trainset = dsets.ImageFolder(root=os.path.join(data_root,'train'), transform=transform_train)
		train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True,drop_last=True,num_workers=train_workers)

		testset = dsets.ImageFolder(root=os.path.join(data_root,'valid'), transform=transform_test)
		test_loader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False,drop_last=True,num_workers=test_workers)
